...
def Login(request, user=None, u="", p=""):
    ...
    if p != "" and str(p) == str(user.password) and len(p) > 0:
        ...
        result = 'false'
    if result == 'true':
        log=Loginlog(user_id=u,ip=ip,time=time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()))
...
def problem(request):
    ...
    return render_to_response('problemset/problem.html',
                              {'problem': problem[0], 'islogin': islogin, 'contestid': contestid}, c)
...
def submit(request):
    ...
    return render_to_response('problemset/submit.html',
                              {'problemid': problemid, 'language': language, 'contestid': contestid}, c)
...
def admin_addcontest_save(request):
    ...
    c = Contest(defunct='C', title=title,
                start_time=datetime.datetime.strptime(starttime, '%Y-%m-%d %H:%M:%S').replace(tzinfo=utc),
                end_time=datetime.datetime.strptime(endtime, '%Y-%m-%d %H:%M:%S').replace(tzinfo=utc),
